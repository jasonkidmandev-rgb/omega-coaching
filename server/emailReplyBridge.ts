...
export async function pollForReplies(): Promise<{
  ...
  // Seal: the IMAP reply bridge reads and auto-processes real client
...
export function startEmailReplyPolling(intervalMs?: number): void {
  // Seal: never start IMAP polling outside production, even via
  // the admin restart endpoint that bypasses the boot-time skip.
  if (sideEffectsDisabled()) {
    console.log('[EmailReplyBridge] non-production — email reply polling disabled');
    return;
  }
