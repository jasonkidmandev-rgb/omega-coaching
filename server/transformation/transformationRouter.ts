...
async function logEnrollmentActivity(database: any, enrollmentId: number, action: string, details: any, performedBy?: string, performedByUserId?: number) {
  try {
    await database.execute(sql`INSERT INTO enrollment_activity_log (enrollment_id, action, details, performed_by, performed_by_user_id) VALUES (${enrollmentId}, ${action}, ${JSON.stringify(details)}, ${performedBy || null}, ${performedByUserId || null})`);
  } catch (err) {
...
async function autoCreateOrLinkClient(
  database: any,
  enrollmentId: number,
  email: string,
